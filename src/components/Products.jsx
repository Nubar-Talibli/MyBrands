...
export default function Products() {
    ...
    useEffect(() => {
        getProducts("https://test.mybrands.az/api/v1/products/");
    }, []);

    useEffect(() => {
        const queryParams = new URLSearchParams();
        if (filters.kateqoriyalar) queryParams.append("categories", filters.kateqoriyalar);
        if (filters.brend) queryParams.append("manufacturers", filters.brend);
    
        const url = `https://test.mybrands.az/api/v1/products/?${queryParams.toString()}`;
        getProducts(url);
      }, [filters]);
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="grid grid-cols-3 mt-8">
                            {products.map((product) => {
                                ...
                                return (
                                    <NavLink to={`${product.product.id}`} key={product.id} className="flex flex-col items-start relative text-sm p-2 cursor-pointer">
                                        <img src={product.image.items[0].file} alt="Product Image" className="h-[400px] mb-4" />
                                        <FontAwesomeIcon icon={faHeart} className="absolute top-[350px] bg-white rounded-full text-xl ml-3 p-2" />
                                        <p>{product.product.title_az}</p>
                                        <div className="flex gap-5">
                                            <p className="flex items-center text-red-600 text-base font-bold mt-5">₼ {discountedPrice.toFixed(2)}</p>
                                            <p className="flex items-center text-gray-500 text-base font-bold line-through mt-5">₼ {product.actual_price}</p>
                                        </div>
                                    </NavLink>
                                )
                            })}
